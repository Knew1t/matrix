...
START_TEST(transpose_matrix_5_input_and_output_are_the_same) {
  matrix_t A = {0};
  matrix_t expected = {0};
  s21_create_matrix(2, 4, &A);
  s21_create_matrix(4, 2, &expected);
  ...
  expected.matrix[3][1] = 0;
  s21_transpose(&A, &A);
  ck_assert_int_eq(s21_eq_matrix(&A, &expected), 1);
  s21_remove_matrix(&A);
  s21_remove_matrix(&expected);
}
END_TEST
...
Suite *transpose_matrix_suite(void) {
  ...
  tcase_add_test(tc, transpose_matrix_5_input_and_output_are_the_same);
